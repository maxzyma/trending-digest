# 聚合门户：生产 cutover runbook

> 生产域 `trending.theuntold.ai` 从「直连 github-trending-digest Pages」切到「trending-diggest 门户 + CF Worker 反代 github-trending」。
> **原子顺序 MUST**：先让新 origin + Worker 就绪并验证，最后切域，防 `/github-trending/*` 断服。
> 本操作在代码 merge 到各仓 main 之后执行。
> 首次切换已于 2026-07 完成；本文档保留为回滚与再次切换的依据。
...
| CF Worker | 无 |
| runbook 依据 | theuntold 侧的私有运维 runbook（CF zone、SSL Full、橙云 + A 记录；zone id 不落公开仓） |
...
- [ ] trending-diggest main：Pages 从 main 构建部署成功，`maxzyma.github.io/trending-diggest/` 可访问
- [ ] github-trending-digest main：baseurl=/github-trending（Pages 重建）
- [ ] theuntold main：Worker 代码已合入
...
2. **部署 Worker**：在 theuntold 侧执行其 edge proxy 部署脚本（`wrangler deploy`）。routes 绑 `trending.theuntold.ai/github-trending*` + legacy。
...
5. **CF DNS 复核**：`trending.theuntold.ai` A → GitHub Pages IP（185.199.108/109/110/111.153 四条），🟠 橙云，SSL/TLS = Full。zone id 见私有 runbook。CF API 调用须 `env -u http_proxy -u https_proxy curl --noproxy '*'`（防代理污染 Authorization）。
...
- 切域回滚：自定义域重新绑回 github-trending-digest + 恢复其 CNAME + baseurl 回 ""（github-trending main 回滚 baseurl 改动）。
- Worker 回滚：在 theuntold 侧 `wrangler rollback` 或删除 routes。

...
每条 SC 对应的验证用例与真实证据位置。TC-ID 形如 `TC-{FORM}-{FOCUS}-{NNN}`（FORM=UI/API，FOCUS=FUNC/BND/ERR）；`scripts/verify-build.sh` 的断言标签按这套编号书写。
...
## 需 CF live 验证层（不可本地静态覆盖）

以下断言依赖 Cloudflare 运行时环境，本地静态测试无法覆盖，只能在 CF 环境实测：

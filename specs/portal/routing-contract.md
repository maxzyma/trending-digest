...
Worker Route 绑定、cache bypass/purge、origin Host/SNI、subrequest 行为、递归防护 —— 标注为 "requires live CF validation"，本地静态测试不覆盖，只能在 CF 环境实测（见 [verification-matrix.md](verification-matrix.md) live-CF 层）。
